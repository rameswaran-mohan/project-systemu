...
def _resolve_capture_sources_mode() -> str:
    """Resolve capture_sources_mode from env with v0.9.34.1 back-compat.

    New var SHARING_ON_CAPTURE_SOURCES (all|single) wins. When it is unset,
    fall back to the legacy SHARING_ON_CAPTURE_SCOPE (broad|narrow) shipped in
    v0.9.34.1, remapping broad->all and narrow->single. Anything invalid or
    absent resolves to "all".
    """
    new = (os.getenv("SHARING_ON_CAPTURE_SOURCES") or "").strip().lower()
    if new in {"all", "single"}:
        return new
    if new:  # set but invalid -> ignore, do NOT fall through to legacy
        return "all"
    legacy = (os.getenv("SHARING_ON_CAPTURE_SCOPE") or "").strip().lower()
    if legacy == "narrow":
        return "single"
...
def _resolve_generalization_mode() -> str:
    ...
    v0.9.35: record-time generalization toggle. DISTINCT from the capture-app
    filter (capture_sources_mode / "sources"). 'standard' (the default) is byte-
    identical to today's behaviour.
    """
    raw = (os.getenv("SYSTEMU_GENERALIZATION") or "").strip().lower()
    if raw in {"broad", "standard", "narrow"}:
        return raw
    return "standard"
...
class Config:
    ...
    # v0.9.35 Phase 0 — capture sources (was Feature D capture-scope). "all"
    # (default) records everything system-wide (unchanged). "single" records
    # only one source app/window; the global input-hook stays system-wide in
    # Phase 1 (see collectors/sources.py). Renamed from capture_scope so the
    # broad/narrow tokens are free for the v0.9.35 generalization toggle.
    capture_sources_mode: str = "all"    # SHARING_ON_CAPTURE_SOURCES: all|single
    capture_source_app: str = ""         # SHARING_ON_CAPTURE_SOURCE_APP: process/app name
    capture_source_title: str = ""       # SHARING_ON_CAPTURE_SOURCE_TITLE: window-title substring

    # v0.9.35 — record-time GENERALIZATION toggle. Controls how abstract a
    # recorded task's intent is + which captured specifics become operator-asked
    # parameters at execution. DISTINCT from capture_sources_mode (Feature D /
    # "sources", which filters WHAT is recorded). "standard" (default) == today's
    # behaviour.
    generalization_mode: str = "standard"    # SYSTEMU_GENERALIZATION: broad|standard|narrow
    ...
    def from_env(cls) -> "Config":
        ...
        instance = cls(
            ...
            capture_sources_mode=_resolve_capture_sources_mode(),
            capture_source_app=(
                os.getenv("SHARING_ON_CAPTURE_SOURCE_APP")
                or os.getenv("SHARING_ON_CAPTURE_APP", "")
            ).strip(),
            capture_source_title=(
                os.getenv("SHARING_ON_CAPTURE_SOURCE_TITLE")
                or os.getenv("SHARING_ON_CAPTURE_TITLE", "")
            ).strip(),
            generalization_mode=_resolve_generalization_mode(),
