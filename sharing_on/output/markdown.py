...
def _render_intent_block(intent: "IntentExtraction") -> str:    # noqa: F821
    ...
    # v0.9.35 (P2): surface generalization mode + lifted parameters so the
    # scroll refiner can carry them onto Scroll.parameters/Scroll.generalization.
    # STANDARD stays silent so older captures render byte-for-byte the same.
    mode = getattr(intent, "generalization", "standard") or "standard"
    if mode != "standard":
        lines.append("")
        lines.append(f"- **Generalization:** {mode}")
    ...
    if params:
        ...
        lines.append("")
        lines.append(
            "_Salient specifics lifted from the capture. At execution the "
            "operator is ASKED for each, pre-filled with the captured value._"
        )
        lines.append("")
        for p in params:
            name = p.get("name", "")
            ...
            kind = p.get("salient_kind")
            desc = p.get("description", "")
            bits = [f"type: {ptype}", req]
            if kind:
                bits.append(f"kind: {kind}")
            lines.append(
                f"- `{name}` ({', '.join(bits)}) — default: `{default}`"
                + (f" — {desc}" if desc else "")
            )
