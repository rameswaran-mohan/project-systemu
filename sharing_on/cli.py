...
def _extract_intent_for_meta(meta, *, steps, events, config):
    """Run intent extraction for an analyze run, threading the record-time
    generalization mode out of session.json (v0.9.35 P2).  Missing/invalid
    key -> 'standard' (handled inside extract_intent)."""
    from sharing_on.analyzer.intent_extractor import extract_intent
    return extract_intent(
        steps=steps,
        events=events,
        session_name=meta.get("name", "Recorded Task"),
        platform_info=meta.get("platform", "Unknown platform"),
        api_key=config.openrouter_api_key,
        base_url=config.openrouter_base_url,
        model=config.tier2_model,
        generalization=meta.get("generalization", "standard"),
    )
...
@click.option(
    ...
    default="all",
    show_default=True,
    help="Capture sources: 'all' records everything; 'single' records only "
         "--source.",
)
@click.option(
    "--scope", "scope_legacy",
    ...
    default=None,
    hidden=True,
    help="Deprecated alias for --sources (broad->all, narrow->single).",
)
@click.option(
    "--source", "source",
    default="",
    help="Single-mode source: app/process name (e.g. chrome.exe) or browser "
         "origin (e.g. https://github.com). Implies --sources single.",
)
@click.option(
    "--app", "app_legacy",
    default="",
...
@click.option(
    "--generalize",
    type=click.Choice(["broad", "standard", "narrow"]),
    default="standard",
    show_default=True,
    help="Generalization at record time: 'broad' abstracts the intent + asks "
         "captured specifics as parameters at run; 'standard' (default) keeps "
         "today's behaviour; 'narrow' bakes specifics in.",
)
@click.pass_context
def record(
    ctx,
    ...
    # v0.9.35 Phase 0: capture-sources overrides. Legacy --scope/--app aliases
    # fold into the new fields (broad->all, narrow->single). --source / --app
    # imply single mode.
    if scope_legacy is not None and sources == "all":
        sources = "single" if scope_legacy == "narrow" else "all"
    src = source or app_legacy
    if src:
        config.capture_source_app = src
        config.capture_sources_mode = "single"
    elif sources == "single":
        config.capture_sources_mode = "single"
    # v0.9.35: record-time generalization toggle (default standard == today).
    config.generalization_mode = generalize
...
def analyze(session_dir: str, model: str):
    ...
    with console.status("[bold]Inferring intent...[/bold]"):
        intent = _extract_intent_for_meta(
            metadata, steps=steps, events=events, config=config,
        )
