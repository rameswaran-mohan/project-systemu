...
def generate_instructions(
    ...
    # callers pass config.tier3_model; this default keeps standalone/test calls
    # on the shipped tier-3 model instead of a stale hardcoded id.
    if model is None:
        import os as _os
        from sharing_on.model_presets import resolve_preset
        model = resolve_preset(_os.environ)["tier3"]
    if not steps:
