...
class TestUserProfileModel:
    def test_required_fields(self):
        ...
        assert p.name == "Rameshwaran"
        assert p.location_text == "Bangalore, India"
...
class TestUserCLICommands:
    ...
    def test_user_init_writes_profile(self, tmp_path, monkeypatch):
        ...
        result = self._invoke(
            ...
            input_text="Rameshwaran\nBangalore, India\nAsia/Kolkata\n/tmp/systemu-out\n",
        )
        assert result.exit_code == 0, result.output
        from systemu.vault.vault import Vault
        prof = Vault(str(vd)).get_user_profile()
        assert prof is not None
        assert prof.name == "Rameshwaran"
        assert prof.location_text == "Bangalore, India"
...
class TestRuntimeUserContext:
    def test_runtime_helper_builds_user_block(self, tmp_path):
        ...
        vlt.save_user_profile(UserProfile(
            name="Rameshwaran", location_text="Bangalore, India",
            timezone="Asia/Kolkata", default_output_dir="/tmp/o"))
        ...
        assert "Rameshwaran" in block
        assert "Bangalore" in block
