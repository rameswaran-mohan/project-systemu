...
| **R-A13.5 ask corpus** `audit/ask_corpus.jsonl` | **only** the shadow exec loop at the ask point (`shadow_runtime` → `replay_metrics.record_ask`) | `APPEND` (O_APPEND line-atomic) | multi-run appenders, whole lines safe | LOW |
