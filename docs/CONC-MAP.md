...
| **World-model facts** `world_model/facts.json` (R-W1 / §5.11.a) | **only** `world_model_populator.populate_from_situation` → `FactStore.put_facts`, on the shadow exec thread's post-survey step (`asyncio.to_thread`, 5s bound) | `A` (whole-file rewrite from one load) | genuinely multi-writer — **concurrent RUNS** are threads of one daemon and each surveys — but the write is IDEMPOTENT-CONVERGENT, not an accumulating RMW: `put_facts` re-derives every row from `fact_id_for(kind, value)`, so two runs surveying the same setup produce the same rows. A lost update costs a re-confirmation on the next survey (`last_confirmed`/an appended `source_chain` step), never a lost FACT. NOT covered: a second daemon process | LOW |
