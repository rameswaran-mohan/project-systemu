...
def render_right_rail(vault, stream_ref: str = "") -> None:
    ...
    with ui.row().classes("items-center").style("gap: 8px; margin-top: 16px;"):
        ...
        # Give the dots an explicit, high-contrast brand color. Without it the
        # spinner falls back to Quasar's default `primary`, which is muted at
        # size="sm" against the dark rail — the field-reported "dots not visible".
