...
- `attempts_before` (integer) — how many already-available tools you tried before concluding none fit (0 if you request without trying anything first). `confidence` (0.0–1.0) — how sure you are this is the right capability to request. Include both on every harness request; they inform the arbitration and the request-quality audit.
