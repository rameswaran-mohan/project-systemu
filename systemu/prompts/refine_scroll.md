...
## Parameters & generalization (v0.9.35)

If the `## Intent` block contains a `### Parameters` subsection and/or a
`- **Generalization:**` line, carry them through faithfully:

- Set top-level `"generalization"` to the mode string ("broad" | "standard" |
  "narrow"). If absent, use "standard".
- Copy each listed parameter into the top-level `"parameters"` array. For each:
  `{"name": ..., "description": ..., "type": "string|number|integer|boolean",
    "default": <the captured value shown after "default:">,
    "salient_kind": ..., "required": true}`.
  The `default` is the CAPTURED value and MUST be preserved verbatim — at
  execution the operator is asked for the slot pre-filled with this value.
- Do NOT bake a parameterised value into an objective's `goal` or `hints`;
  keep it ONLY in `parameters`. (Narrow/standard captures have no parameters —
  emit `"parameters": []`.)
...
```json
{
  ...
  "tags": ["tag1", "tag2"],
  "generalization": "broad | standard | narrow (default standard)",
  "parameters": [
