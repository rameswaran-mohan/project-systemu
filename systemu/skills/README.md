...
This mirrors the Hermes + Odysseus pattern: agents ship generic CAPABILITIES (file ops, web fetch, terminal, send_message, etc. — see `systemu/runtime/tools/` for the v0.9.3+ code-registered batch), then build a skill library from successful runs. Recipes that anticipate use cases will overfit; recipes that emerge from real usage scale.
