...
from systemu.core.models import (
    ActionBlock, Objective, Scroll, ScrollParameter, ScrollStatus, TraceEvent,
)
...
# ─── v0.9.35: generalization → Scroll.parameters projection ─────────────────

_VALID_SCROLL_GENERALIZATION = ("broad", "standard", "narrow")
_VALID_PARAM_TYPES = ("string", "number", "integer", "boolean")


def _coerce_scroll_generalization(value: Any) -> Optional[str]:
    """Map the refiner's generalization string onto Scroll.generalization.
    None == standard == today's behaviour, so 'standard' and invalid both
    collapse to None (the additive field's default)."""
    v = str(value or "").strip().lower()
    if v in ("broad", "narrow"):
        return v
    return None   # standard / missing / invalid


def _build_scroll_parameters(result: Dict[str, Any]) -> List[ScrollParameter]:
    """Project result['parameters'] onto validated ScrollParameter rows.
    Only broad mode carries parameters; nameless rows are dropped; invalid
    types fall back to 'string'; required defaults true (pinned contract)."""
    if _coerce_scroll_generalization(result.get("generalization")) != "broad":
        return []
    out: List[ScrollParameter] = []
    for raw in (result.get("parameters") or [])[:12]:
        ...
        try:
            out.append(ScrollParameter(
                ...
                enum=([str(e)[:120] for e in raw["enum"]][:25]
                      if raw.get("enum") else None),
                format=(str(raw["format"])[:32] if raw.get("format") else None),
                required=bool(raw.get("required", True)),
            ))
        except Exception as exc:
            logger.warning("[Scroll] dropping malformed parameter %r: %s", raw, exc)
    return out
...
def refine_scroll(
    ...
    *,
    ...
    scroll = Scroll(
        ...
        generalization=_coerce_scroll_generalization(result.get("generalization")),
        parameters=_build_scroll_parameters(result),
    )
