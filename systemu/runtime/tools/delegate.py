...
Hermes pattern: the agent calls this tool to decompose a goal and dispatch
sub-goals to forked Tier-1 subagents. Each child:
...
def _compute_child_whitelist(parent_whitelist: Set[str]) -> Set[str]:
    """Hermes invariant: child sees parent_whitelist minus delegate-class tools.
...
def _delegate_dynamic_schema():
    """Hermes pattern: surface current Config limits in the tool description
    so the model sees accurate constraints at LLM-render time."""
    from sharing_on.config import Config
    try:
        cfg = Config.from_env()
